...
def generate_ID_MW_file(molecular_network_df, output_filename):
    ...
    with tqdm(total=2, desc="Extracting unique IDs and MWs") as pbar:
        ...
        source_df = molecular_network_df[['source', 'source_MW']].rename(columns={'source': 'ID', 'source_MW': 'MW'})
        target_df = molecular_network_df[['target', 'target_MW']].rename(columns={'target': 'ID', 'target_MW': 'MW'})
        pbar.update(1)
...
def search_candidates_by_mw(input_data, pubchem_dict, candidates_folder, ppm_threshold):
    ...
    pubchem_mws = [(cid, entry['mw'], entry['smiles'], entry['formula']) for cid, entry in tqdm(pubchem_dict.items(), desc="Unpacking pubchem dict")]
...
def filter_candidates_by_formula(input_folder, output_folder, is_filter_element=True, element_set=None):
...
if __name__ == "__main__":
    ...
    unique_ids_df=generate_ID_MW_file(molecular_network_df, os.path.join(tmp_result_path, "unique_ID_MW.csv"))
    ...
    search_candidates_by_mw(unique_ids_df, pubchem_dict, tmp_candidates_folder, args.ppm_threshold)

    # 依据分子式进行元素过滤，得到candidates
    tmp_filtered_candidates_by_formula_folder= tmp_result_path + "candidates_filtered_by_formula"
    filter_candidates_by_formula(tmp_candidates_folder, tmp_filtered_candidates_by_formula_folder, args.is_filter_element, args.element_set)
