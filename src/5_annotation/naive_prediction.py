...
def pick_cycle_annotation(unique_folder, not_unique_folder, naive_prediction_folder, threshold_tanimoto_similarity):
    # 初始化结果列表
    result_rows = []
    ...
    for file in unique_files + not_unique_files:
        src_path = os.path.join(unique_folder if file in unique_files else not_unique_folder, file)
...
if __name__ == "__main__":
    ...
    unique_folder="tmp/Seednode_and_Targetnode_Morgan_Similarity_score_split_unique_Top10"
    not_unique_folder="tmp/Seednode_and_Targetnode_Morgan_Similarity_score_split_not_unique_Top10"
    naive_prediction_folder="tmp/naive_prediction_results"
    os.makedirs(naive_prediction_folder, exist_ok=True)
    
    # 运行 naive prediction
    final_df = pick_cycle_annotation(unique_folder, not_unique_folder, naive_prediction_folder, threshold_tanimoto_similarity)
