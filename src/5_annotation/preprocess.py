...
def split_target_node(unique_csv, not_unique_csv, seed_target_tanimoto_folder, unique_folder, not_unique_folder):
    ...
    all_files = [f for f in os.listdir(seed_target_tanimoto_folder) if f.endswith('.csv')]
...
def pickle_top10_unique_seednode(unique_folder):
    ...
    csv_files = [f for f in os.listdir(unique_folder) if f.endswith('.csv') and '_' in f]
...
def pickle_top10_not_unique(not_unique_csv, not_unique_folder):
    # 路径设置（请修改为你实际的路径）
    ...
    # 遍历文件夹B
    csv_files = [f for f in os.listdir(not_unique_folder) if f.endswith('.csv') and '_' in f]
...
if __name__ == "__main__":
    ...
    edges_df=generate_seednode_and_targetnode(molecular_network_df, seednode_df, os.path.join(tmp_result_path, "Seednode_and_Targetnode.csv"))
    ...
    not_unique_csv = tmp_result_path+'Seednode_and_Targetnode_not_unique_seednodes.csv'
    seed_target_tanimoto_folder = tmp_result_path+'Seednode_and_Targetnode_Morgan_Similarity_score'
    unique_folder = 'tmp/Seednode_and_Targetnode_Morgan_Similarity_score_split_unique'
    not_unique_folder = 'tmp/Seednode_and_Targetnode_Morgan_Similarity_score_split_not_unique'
    os.makedirs(seed_target_tanimoto_folder, exist_ok=True)
    os.makedirs(unique_folder, exist_ok=True)
    os.makedirs(not_unique_folder, exist_ok=True)
    
    split_target_node(unique_csv, not_unique_csv, seed_target_tanimoto_folder,unique_folder,not_unique_folder)
    
    pickle_top10_unique_seednode(unique_folder)
    pickle_top10_not_unique(not_unique_csv, not_unique_folder)
