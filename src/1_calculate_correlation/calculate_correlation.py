    ...
    with multiprocessing.Pool(processes=jobs) as pool:
        for i, row in enumerate(arr):
            if i > 0:
                ...
                for j in range(i):
                    corr_arr[i][j], pval_arr[i][j], non_zero_count_arr[i][j] = results[
                        j
                    ]
            pbar.update(i)
...
def save_to_csv(correlation_results, csv_filename):
    f_corr_df, f_pv_df, f_n_df = (
        correlation_results["corr"],
        correlation_results["p_value"],
        correlation_results["non_zero_count"],
    )
            ...
            for j, substance2 in enumerate(substances):
                if j < i:  # 只输出下三角部分
                    ...
                    combined_data.append(
                        [substance1, substance2, correlation, p_value, n]
                    )
    ...
    combined_df = pd.DataFrame(
        combined_data,
        columns=["Substance 1", "Substance 2", "Correlation", "P-Value", "n"],
    )
...
def compare_filtered_unfiltered(df1, df2, output_filename):
    ...
    # 将不同的相关系数及P值写入CSV文件
    different_correlations_df = pd.DataFrame(
        different_correlations,
        columns=[
            "Substance 1",
            "Substance 2",
...
def fisher_z_transform(r, n):
    ...
    return 0.5 * (
        pd.Series(r).apply(lambda x: 0.5 * (np.log(1 + x) - np.log(1 - x)))
        / np.sqrt(1 / (n - 3))
    )
...
def exec_fish_z(df, output_filename):
    ...
    with tqdm(total=total_iterations, desc="Processing Rows") as pbar:
        # 进行Fisher's z变换
        df["z1"] = np.where(
            df["n1"] <= 3, np.nan, fisher_z_transform(df["Correlation 1"], df["n1"])
        )
        df["z2"] = np.where(
            df["n2"] <= 3, np.nan, fisher_z_transform(df["Correlation 2"], df["n2"])
        )
...
def replace_with_miniCor(unfiltered_df, significant_df, output_filename):
    ...
    merged_df = pd.merge(
        unfiltered_df, significant_df, on=["Substance 1", "Substance 2"], how="left"
    )
    ...
    merged_df["Correlation_min"] = merged_df["Correlation 1"].where(
        mask, merged_df["Correlation 2"]
    )
    merged_df["P-Value_min"] = merged_df["P-Value 1"].where(
        mask, merged_df["P-Value 2"]
    )
    ...
    with tqdm(total=merged_df.shape[0]) as pbar:
        merged_df[["Substance 1", "Substance 2", "Correlation", "P-Value"]].to_csv(
...
if __name__ == "__main__":
    ...
    ), "The shape read from the file should be (number of substances, number of samples)"
