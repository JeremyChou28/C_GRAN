...
def merge_source_target_cor(source_target_df, correlation_df, output_filename):
    ...
    source_target_df['pair_key'] = source_target_df[['source', 'target']].min(axis=1).astype(str) + '_' + source_target_df[['source', 'target']].max(axis=1).astype(str)
    correlation_df['pair_key'] = correlation_df[['Substance 1', 'Substance 2']].min(axis=1).astype(str) + '_' + correlation_df[['Substance 1', 'Substance 2']].max(axis=1).astype(str)
    ...
    with tqdm(total=1, desc="Merging correlation data") as pbar:
        merged = pd.merge(source_target_df, correlation_df_slim, on='pair_key', how='left')
        pbar.update(1)
...
if __name__ == "__main__":
    ...
    source_target_cor_df = merge_source_target_cor(source_target_df, correlation_df, source_target_cor_filename)
